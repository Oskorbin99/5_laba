...
BodyJS = [$("div#BodyImgText"),$('img#Img_body'),$('p#text_body'),$('p#mus1'),$('audio#Audiomus1'),document.querySelector('p#mus2'),document.querySelector('audio#Audiomus2') ];
	...
	try {
		...
		con.connect(function (err) {
			...
			con.query("SELECT * FROM s_data_js ", function (err, result, fields) {
				if (err) throw err;
				BodyJS[0].text(result[test].nameOsnova);
				BodyJS[1].attr("src", result[test].ImgOsnova);
				BodyJS[2].text(result[test].TextOsnova);
				BodyJS[3].text(result[test].Namemus1);
				BodyJS[4].attr("src", result[test].Audiomus);
				BodyJS[5].innerHTML = result[test].Namemus1;
				BodyJS[6].src = result[test].Audiomus1;
			});
		});
	}
	catch (err) {
